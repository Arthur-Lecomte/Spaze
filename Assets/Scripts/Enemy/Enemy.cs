...
public class Enemy : MonoBehaviour, ICanTakeDamage {
    ...
    protected virtual void ScaleStats() {
        ...
        shootInterval = Mathf.Max(0.1f, shootInterval * (1 - (Level - 1) * 0.05f)); // Diminue de 5% par niveau (min = 0.1f)
        moveSpeed *= (1 + (Level - 1) * 0.05f); // Augmente de 5% par niveau
    }

    /// <summary>
    /// Obtient les dégâts infligés par l'ennemi.
    /// </summary>
    /// <returns>Les dégâts infligés par l'ennemi.</returns>
